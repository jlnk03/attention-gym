"""Benchmark for Retrieval-in-Network (RIN) Cross-Attention Write Mask"""
...
def generate_rin_write_mask_mod(
    img_lengths: List[int], latent_len: int, device: str = "cuda"
):
    """
    Generate a mask mod for RIN Cross-Attention write operation.
    
    Each image i contains:
      • img_lengths[i] tape tokens (Q side)
      • latent_len key/value tokens (KV side)
    Queries may only see KV tokens from the same image.
    """
    # cumulative boundaries of the tape for the query side: [0, L0, L0+L1, ...]
    boundaries = torch.tensor([0, *accumulate(img_lengths)], device=device, dtype=torch.long)
    total_q = int(boundaries[-1].item())

    def mask_mod(b, h, q_idx, kv_idx):
        # image id for tape query (variable size -> searchsorted)
        q_idx = torch.clamp(q_idx, 0, total_q - 1)
        img_q = torch.searchsorted(boundaries, q_idx, right=True) - 1
        # image id for latent kv token (fixed size per image)
        img_kv = kv_idx // latent_len
        return img_q == img_kv

    return mask_mod


def run_rin_write_benchmark(
    n_images: int = 16,
    latent_len: int = 128,
    min_tokens: int = 64,
    max_tokens: int = 512,
    ...
    """
    Benchmark RIN Cross-Attention Write operation with different attention implementations.
    ...
    Q = sum(img_lengths)      # Total query tokens (tape)
    KV = n_images * latent_len  # Total key/value tokens (latent)
    
    # Generate the mask mod for write operation
    mask_mod = generate_rin_write_mask_mod(img_lengths, latent_len, device)
    ...
    # Print results
    print_header(f"RIN Cross-Attention Write Benchmark (B={B}, n_images={n_images})")
    print(
        tabulate(
            rows,
            headers=[
...
def main(
    ...
    """
    Run RIN Cross-Attention Write benchmark with configurable parameters.
    """
    # Always run with batch size of 1 as requested
    run_rin_write_benchmark(
        n_images=n_images,
