"""Benchmark for Retrieval-in-Network (RIN) Cross-Attention Mask using torch.utils.benchmark"""
...
def run_rin_benchmark(
    ...
    Args:
        ...
        num_warmup: Number of warmup iterations for benchmark
        num_repeats: Number of measurement iterations for benchmark
    """
    ...
    timer_fw = Timer(
        stmt="func()",
        globals={"func": sdpa_mask},
        num_threads=1,
    )
    fw_benchmark = timer_fw.timeit(num_repeats)
    fw_time = fw_benchmark.mean * 1000  # Convert to ms
    
    out = sdpa_mask()
    
    timer_bw = Timer(
        stmt="out.backward(grad_out, retain_graph=True)",
        globals={"out": out, "grad_out": grad_out},
        num_threads=1,
    )
    bw_benchmark = timer_bw.timeit(num_repeats)
    bw_time = bw_benchmark.mean * 1000  # Convert to ms
    ...
    print(f"Benchmark config: {num_repeats} iterations (mean time reported)")
